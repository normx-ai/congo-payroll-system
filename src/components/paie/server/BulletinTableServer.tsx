  ...
  return (
    <div className="border border-black">
      ...
      <div className="grid bg-gray-100 border-b border-black text-center font-bold text-xs"
        ...
        <div className="border-r border-black p-0.5 flex items-center justify-center">n°</div>
        <div className="border-r border-black p-0.5 flex items-center justify-center">désignation</div>
        <div className="border-r border-black p-0.5 flex items-center justify-center">qté</div>
        <div className="border-r border-black p-0.5 flex items-center justify-center">base</div>
        <div className="border-r border-black p-0.5" style={{gridColumn: 'span 3'}}>part salariale</div>
        <div className="p-0.5" style={{gridColumn: 'span 2'}}>part patronale</div>
      </div>

      <div className="grid bg-gray-100 border-b border-black text-center font-bold text-xs"
           style={{gridTemplateColumns: '40px 250px 50px 70px 50px 70px 70px 50px 70px'}}>
        <div style={{gridColumn: 'span 4'}}></div>
        <div className="border-r border-black p-0.5">taux(%)</div>
        <div className="border-r border-black p-0.5">gain</div>
        <div className="border-r border-black p-0.5">retenue</div>
        <div className="border-r border-black p-0.5">taux(%)</div>
        <div className="p-0.5">retenue</div>
      </div>
        ...
        .map((gain, index) => {
          ...
          return (
            <div key={`gain-soumis-${index}`} className="grid border-b border-gray-300 text-xs"
                 style={{gridTemplateColumns: '40px 250px 50px 70px 50px 70px 70px 50px 70px'}}>
              <div className="border-r border-black p-0.5 text-center">{gain.code}</div>
              <div className="border-r border-black p-1">{gain.designation}</div>
              <div className="border-r border-black p-0.5 text-center">
                {(['0100', '0110', '0120'].includes(gain.code)) ? (gain.quantite || '') : ''}
              </div>
              <div className="border-r border-black p-0.5 text-right">
                {gain.code === '1010' ? formatAmount(gain.base || 0) : ''}
              </div>
              <div className="border-r border-black p-0.5 text-center">{gain.code === '1010' ? (gain.taux?.replace('%', '') || '') : ''}</div>
      ...
      {calculation.rubriques.retenues
        ...
        .map((retenue, index) => (
          <div key={`cotisation-${index}`} className="grid border-b border-gray-300 text-xs"
               style={{gridTemplateColumns: '40px 250px 50px 70px 50px 70px 70px 50px 70px'}}>
            <div className="border-r border-black p-0.5 text-center">{retenue.code}</div>
            <div className="border-r border-black p-1">{retenue.designation}</div>
            <div className="border-r border-black p-0.5 text-center"></div>
            <div className="border-r border-black p-0.5 text-right">{retenue.base ? formatAmount(retenue.base) : ''}</div>
            {['3100'].includes(retenue.code) ? (
              <>
                <div className="border-r border-black p-0.5 text-center">4</div>
                <div className="border-r border-black p-1"></div>
                <div className="border-r border-black p-0.5 text-right">{formatAmount(retenue.montant * (4/12))}</div>
              </>
            ) : ['3510', '3540', '3550', '3560', '3570'].includes(retenue.code) ? (
              <>
                <div className="border-r border-black p-0.5 text-center">{retenue.taux?.replace('%', '') || ''}</div>
                <div className="border-r border-black p-1"></div>
                <div className="border-r border-black p-0.5 text-right">{formatAmount(retenue.montant)}</div>
              </>
            ) : (
              <>
                <div className="border-r border-black p-0.5 text-center"></div>
            ...
            {['3100'].includes(retenue.code) ? (
              <>
                <div className="border-r border-black p-0.5 text-center">8</div>
                <div className="p-0.5 pr-1 text-right">{formatAmount(retenue.montant * (8/12))}</div>
              </>
            ) : ['3110', '3120', '3530', '3130'].includes(retenue.code) ? (
              <>
                <div className="border-r border-black p-0.5 text-center">{retenue.taux?.replace('%', '') || ''}</div>
                <div className="p-0.5 pr-1 text-right">{formatAmount(retenue.montant)}</div>
              </>
            ) : ['3510', '3540', '3550', '3560', '3570'].includes(retenue.code) ? (
              <>
                <div className="border-r border-black p-0.5 text-center"></div>
                <div className="p-0.5 pr-1 text-right"></div>
              </>
            ) : (
              <>
                <div className="border-r border-black p-0.5 text-center">{retenue.taux?.replace('%', '') || ''}</div>
      ...
      <div className="grid border-b border-black text-xs font-bold bg-gray-50"
        ...
        <div className="border-r border-black p-1"></div>
        <div className="p-0.5 pr-1 text-right">
          {formatAmount(
            calculation.rubriques.retenues
              .filter(retenue => retenue.code === '3100')
        ...
        .map((item, index) => {
          ...
          return (
            <div key={`non-soumis-${index}`} className="grid border-b border-gray-300 text-xs"
                 style={{gridTemplateColumns: '40px 250px 50px 70px 50px 70px 70px 50px 70px'}}>
              <div className="border-r border-black p-0.5 text-center">{item.code}</div>
              <div className="border-r border-black p-1">{item.designation}</div>
              <div className="border-r border-black p-0.5 text-center">{item.code === '5010' ? '' : ((item as { quantite?: number }).quantite || '')}</div>
              <div className="border-r border-black p-0.5 text-right"></div>
              <div className="border-r border-black p-0.5 text-center">{(item as { taux?: string }).taux?.replace('%', '') || ''}</div>
